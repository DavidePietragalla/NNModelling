...
\raggedright\par{\large{
		Il progetto prevede l'implementazione di una
		DSL (Domain Specific Language) per poter modellare reti neurali
		usando un linguaggio diagrammatico. Una volta modellato, il nostro software
		permette di poter istanziare la rete in pytorch e di poter fare training della rete
		su un determinato dataset dato dall'utente.

		È stato usato un approccio Model-Driven.

		Prima è stata definita la sintassi della DSL
		utilizzando un meta-modello UML. Finita la fase di Design
		è stato implementata un applicazione front-end che permette di
		poter disegnare i modelli.

		I modelli sono salvati vengono convertiti in una configurazione compatibile
		con il framework python "OmegaConf".

		Data la configurazione, è possibile scegliere ottimizzatore, scheduler
		e altri parametri come il seed.

		Può essere effettuato il training della rete e i plot vengono creati utilizzando
		il servizio Weight \& Biases
	}}
\vspace{10pt}
\newpage
