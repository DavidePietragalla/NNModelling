...
\begin{frame}{UML Metamodel}
	Three complementary views:
	\begin{enumerate}
		\item \alert{Node Hierarchy} -- modelling elements (Module, Input, Join, Subflow)
		\item \alert{Connection Model} -- Handle-based connection with implicit forks / explicit joins
		\item \alert{Stereotype System} -- extensible layer definitions via JSON stereotypes
	\end{enumerate}

	\vspace{10pt}
	Key design: \alert{recursive subflows} enable hierarchical decomposition
\end{frame}
...
\begin{frame}{Dynamic Model Execution}
	\begin{itemize}
		\item \texttt{LightningModule} builds architecture from Hydra config at runtime
		\item Forward pass uses \alert{BFS traversal with in-degree tracking}
		\item Join nodes collect all inputs ordered by \texttt{targetHandle}
		\item Custom operations: Subflow, Repeat, HorizontalRepeat
	\end{itemize}

	\vspace{10pt}
	Result: \alert{179 tests} across front-end and back-end
\end{frame}
...
\begin{frame}{Example: Transformer Classifier}
	\centering
	Hierarchical DAG spanning three levels:
	\begin{itemize}
	...
	\vspace{10pt}
	\textbf{Result}: 98.4\% accuracy on EnronSpam dataset
\end{frame}
...
\begin{frame}{Achievements}
	\begin{itemize}
		...
		\item Model-driven approach with formal UML metamodel
